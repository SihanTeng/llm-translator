...
int main(int argc, char *argv[]) {
    // Tests/e2e can isolate settings away from the user's real config.
    // (QSettings UserScope follows XDG_CONFIG_HOME on Linux; setPath() on
    // IniFormat does NOT redirect NativeFormat lookups. Set before any Qt
    // object exists, since path resolution is cached early.)
