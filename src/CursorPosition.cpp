...
QPoint globalCursorPosition() {
    ...
    if (desktop.contains("Hyprland")) {
        return runHelper(
            QStringLiteral("hyprctl"), { QStringLiteral("cursorpos") }, [](const QByteArray &out) {
                // Format: "x, y"
    ...
    if (desktop.contains("sway")) {
        return runHelper(QStringLiteral("swaymsg"),
            { QStringLiteral("-t"), QStringLiteral("get_seats") }, [](const QByteArray &out) {
